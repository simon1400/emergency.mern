...
export default class Create extends Component {
  ...
  submitForm = (e) => {
    ...
    axios.post('http://localhost:4000/admin/tests/create', this.state)
          ...
          <div className="uk-grid uk-child-width-1-1" uk-grid="">
            <div>
              <form >
                ...
                <h3 className="uk-legend">Otazka {this.state.currentQuestion + 1}</h3>
                ...
                {this.state.questions[this.state.currentQuestion].descriptionShow ? 
                  ...
                  {this.state.questions[this.state.currentQuestion].asks.map((ask, index) => (
                    <div key={index} className="uk-grid uk-grid-small" uk-grid="">
                      <div className="uk-margin uk-width-4-5">
                        <input
                          ...
                          placeholder="Odpoved"
                          value={this.state.questions[this.state.currentQuestion].asks[index].nameAsk}
                        />
                      </div>
                      <div className="uk-width-1-5 uk-flex uk-flex-between">
                        <input
                          ...
                          data-countitem={index}
                          placeholder="Body"
                          value={this.state.questions[this.state.currentQuestion].asks[index].valueAsk}
                        />
                      <span data-countparent={this.state.currentQuestion} data-index={index} onClick={this.deleteAsk}>minus</span>
                      </div>
                    </div>
                  ))}
                  ...
                  <button className="uk-button uk-button-primary uk-align-right" onClick={this.addQuestion}>Next question</button>
                  {this.state.currentQuestion ? <button className="uk-button uk-button-primary uk-align-right" onClick={this.prevQuestion}>Prev question</button> : ''}

                  <button className="uk-button uk-button-primary" onClick={this.submitForm}>Save</button>
                </fieldset>
              </form>
            </div>
          </div>
