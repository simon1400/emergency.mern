...
class ChangeStream extends EventEmitter {
  ...
  next(callback) {
    ...
    if (this.isClosed()) {
      if (callback)
        return callback(new Error("Change Stream is not open."), null);
      return self.promiseLibrary.reject(
        new Error("Change Stream is not open.")
      );
    }
    ...
    return this.cursor
      .next()
      .then(change =>
        processNewChange({ changeStream: self, change, callback })
      )
      .catch(error =>
        processNewChange({ changeStream: self, error, callback })
      );
  }
  ...
  unpipe(destination) {
    if (
      this.pipeDestinations &&
      this.pipeDestinations.indexOf(destination) > -1
    ) {
      this.pipeDestinations.splice(
        this.pipeDestinations.indexOf(destination),
        1
      );
    }
...
function getStartAtOperationTime(self) {
  ...
  return (
    ...
    self.options.startAtOperationTime
  );
}
...
var buildChangeStreamAggregationCommand = function(self) {
  ...
  var command = {
    aggregate:
      self.type === CHANGE_DOMAIN_TYPES.COLLECTION ? namespace.collection : 1,
...
function processNewChange(args) {
  ...
  if (error) {
    if (isResumableError(error) && !changeStream.attemptingResume) {
      ...
      if (
        !(getResumeToken(changeStream) || getStartAtOperationTime(changeStream))
      ) {
        const startAtOperationTime =
          changeStream.cursor.cursorState.operationTime;
