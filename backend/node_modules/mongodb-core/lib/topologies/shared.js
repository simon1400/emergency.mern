...
function createClientInfo(options) {
  ...
  if (
    clientInfo.platform &&
    clientInfo.platform.indexOf("mongodb-core") === -1
  ) {
    clientInfo.platform = f(
      "%s, mongodb-core: %s",
      clientInfo.platform,
      driverVersion
    );
  ...
  if (options.appname) {
    ...
    var appname =
      buffer.length > 128
        ? buffer.slice(0, 128).toString("utf8")
        : options.appname;
  ...
  return function(callback) {
    ...
    self.command(
      ...
      function(err, r) {
        ...
        self.s.inquireServerStateTimeout = setTimeout(
          inquireServerState(self),
          self.s.haInterval
        );
      }
    );
...
function diff(previous, current) {
  ...
  for (i = 0; i < previous.servers.length; i++) {
    ...
    for (j = 0; j < current.servers.length; j++) {
      ...
      // Matching server
      if (
        prevServer.address.toLowerCase() === currServer.address.toLowerCase()
      ) {
