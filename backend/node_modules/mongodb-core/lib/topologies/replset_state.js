...
ReplSetState.prototype.update = function(server) {
  ...
  if (ismaster) {
    ...
    hosts = hosts.concat(
      Array.isArray(ismaster.arbiters) ? ismaster.arbiters : []
    );
    hosts = hosts.concat(
      Array.isArray(ismaster.passives) ? ismaster.passives : []
    );
  ...
  if (
    ismaster.logicalSessionTimeoutMinutes !== undefined &&
    !isArbiter(ismaster)
  ) {
    if (
  ...
  if (!this.primary && ismaster.ismaster && ismaster.setName) {
    ...
    var currentElectionId =
      self.set[self.primary.name.toLowerCase()].electionId;
    var currentSetVersion =
      self.set[self.primary.name.toLowerCase()].setVersion;
  ...
  if (
    this.set[serverName] &&
    this.set[serverName].type === ServerType.RSPrimary
  ) {
    self.emit("left", "primary", this.primary);
...
ReplSetState.prototype.pickServer = function(readPreference) {
  ...
  if (
    ...
    return new MongoError(
      "primary readPreference incompatible with maxStalenessSeconds"
    );
  }
...
var filterByTags = function(readPreference, servers) {
  ...
  var filteredServers = [];
  var tagsArray = Array.isArray(readPreference.tags)
    ? readPreference.tags
    : [readPreference.tags];
...
function pickNearestMaxStalenessSeconds(self, readPreference) {
  ...
  if (maxStalenessMS < 90 * 1000) {
    return new MongoError(
      "maxStalenessSeconds must be set to at least 90 seconds"
    );
  }
...
function inList(ismaster, server, list) {
  for (var i = 0; i < list.length; i++) {
    if (
      list[i] &&
      list[i].name &&
      list[i].name.toLowerCase() === server.name.toLowerCase()
    )
      return true;
  }
