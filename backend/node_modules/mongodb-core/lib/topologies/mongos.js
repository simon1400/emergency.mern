  ...
  return function() {
      ...
      if (self.ismaster.msg === "isdbgrid") {
        ...
        for (let i = 0; i < self.connectedProxies.length; i++) {
          if (self.connectedProxies[i].name === _this.name) {
            // Move from connectingProxies
            moveServerFrom(
              self.connectingProxies,
              self.disconnectedProxies,
              _this
            );
            // Emit the initial topology
            emitTopologyDescriptionChanged(self);
        ...
        if (self.s.logger.isWarn()) {
          var message =
            "expected mongos proxy, but found replicaset member mongod for server %s";
          // We have a standalone server
          if (!self.ismaster.hosts) {
            message =
              "expected mongos proxy, but found standalone mongod for server %s";
          }
    ...
    if (self.connectingProxies.length === 0) {
      ...
      if (self.connectedProxies.length > 0 && self.state === CONNECTING) {
        ...
        return self.emit(
          "error",
          new MongoError("no mongos proxies found in seed list")
        );
      }
