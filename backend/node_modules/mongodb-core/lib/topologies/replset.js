...
const isRetryableWritesSupported = require("./shared")
  ...
  server.command(
    ...
    function(err, r) {
      ...
      if (err) {
        ...
        if (
          server.ismaster.lastWrite &&
          server.ismaster.lastWrite.lastWriteDate
        ) {
        ...
        if (server.lastIsMasterMS === -1) {
          ...
          server.lastIsMasterMS =
            0.2 * latencyMS + (1 - 0.2) * server.lastIsMasterMS;
        }
      ...
      self.s.replicaSetState.updateServerMaxStaleness(
        server,
        self.s.haInterval
      );
...
function topologyMonitor(self, options) {
  ...
  if (_process === Timeout) {
    return connectNewServers(
      self,
      self.s.replicaSetState.unknownServers,
      function(err) {
    ...
    return function() {
      ...
      connectNewServers(
        ...
        function() {
          ...
          // Create a timeout
          self.intervalIds.push(
            new Timeout(executeReconnect(self), monitoringFrequencey).start()
          );
        }
      );
    };
  ...
  self.intervalIds.push(
    new Timeout(executeReconnect(self), intervalTime).start()
  );
}
...
function handleEvent(self, event) {
  return function() {
    ...
    if (self.s.logger.isDebug()) {
      self.s.logger.debug(
        f(
          "handleEvent %s from server %s in replset with id %s",
          event,
          this.name,
          self.id
        )
...
function shouldTriggerConnect(self) {
  ...
  const secondaryOnlyConnectionAllowed =
    self.s.options.secondaryOnlyConnectionAllowed;
  ...
  return (
    (isConnecting &&
      ((readPreferenceSecondary && hasSecondary) ||
        (!readPreferenceSecondary && hasPrimary))) ||
...
ReplSet.prototype.remove = function(ns, ops, options, callback) {
  // Execute write operation
  executeWriteOperation(
    { self: this, op: "remove", ns, ops },
    options,
    callback
  );
};

const RETRYABLE_WRITE_OPERATIONS = [
  "findAndModify",
  "insert",
  "update",
  "delete"
...
ReplSet.prototype.command = function(ns, cmd, options, callback) {
  ...
  if (server == null) {
    return callback(
      new MongoError(
        f(
          "no server found that matches the provided readPreference %s",
          readPreference
        )
      )
    );
