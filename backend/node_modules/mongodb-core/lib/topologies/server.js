...
var Server = function(options) {
  ...
  this.s = {
    ...
    monitoring:
      typeof options.monitoring === "boolean" ? options.monitoring : true,
    // Is the server in a topology
    inTopology: !!options.parent,
    // Monitoring timeout
    monitoringInterval:
      typeof options.monitoringInterval === "number"
...
function disconnectHandler(self, type, ns, cmd, options, callback) {
  ...
  if (!self.s.pool.isConnected()) {
    callback(
      new MongoError(f("no connection available to server %s", self.name))
    );
    return true;
  }
}
...
var eventHandler = function(self, event) {
  return function(err, conn) {
    ...
    if (event === "connect") {
      ...
      if (
        serverAccounting &&
        ["close", "timeout", "error", "parseError", "reconnectFailed"].indexOf(
          event
        ) !== -1
      ) {
      ...
      if (
        ...
        return self.emit(
          ...
          new MongoNetworkError(
            f(
              "failed to connect to server [%s] on first connect [%s]",
              self.name,
              err
            )
          )
...
Server.prototype.connect = function(options) {
  ...
  if (
    self.s.pool &&
    !self.s.pool.isDisconnected() &&
    !self.s.pool.isDestroyed()
  ) {
    throw new MongoError(
      f("server instance in invalid state %s", self.s.pool.state)
    );
  }

  // Create a pool
  self.s.pool = new Pool(
    this,
    Object.assign(self.s.options, options, { bson: this.s.bson })
  );
...
Server.prototype.command = function(ns, cmd, options, callback) {
  ...
  if (collationNotSupported(this, cmd)) {
    return callback(
      new MongoError(`server ${this.name} does not support collation`)
    );
  }
