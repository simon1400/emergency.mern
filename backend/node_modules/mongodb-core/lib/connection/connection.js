...
const MESSAGE_HEADER_SIZE = require("../wireprotocol/shared")
  .MESSAGE_HEADER_SIZE;
...
class Connection extends EventEmitter {
  ...
  constructor(socket, options) {
    ...
    this.maxBsonMessageSize =
      options.maxBsonMessageSize || DEFAULT_MAX_BSON_MESSAGE_SIZE;
    ...
    this.keepAlive =
      typeof options.keepAlive === "boolean" ? options.keepAlive : true;
  ...
  write(buffer) {
    ...
    if (this.logger.isDebug()) {
      if (!Array.isArray(buffer)) {
        this.logger.debug(
          `writing buffer [${buffer.toString("hex")}] to ${this.address}`
        );
      } else {
        for (let i = 0; i < buffer.length; i++)
          this.logger.debug(
            `writing buffer [${buffer[i].toString("hex")}] to ${this.address}`
          );
      }
    }
...
function errorHandler(conn) {
  return function(err) {
    ...
    if (conn.logger.isDebug()) {
      conn.logger.debug(
        `connection ${conn.id} for [${
          conn.address
        }] errored out with [${JSON.stringify(err)}]`
      );
    }
...
function timeoutHandler(conn) {
  return function() {
    ...
    if (conn.logger.isDebug()) {
      conn.logger.debug(
        `connection ${conn.id} for [${conn.address}] timed out`
      );
    }
    ...
    conn.emit(
      ...
      new MongoNetworkError(
        `connection ${conn.id} to ${conn.address} timed out`
      ),
      conn
    );
  };
...
function closeHandler(conn) {
  return function(hadError) {
    ...
    if (conn.logger.isDebug()) {
      conn.logger.debug(
        `connection ${conn.id} with for [${conn.address}] closed`
      );
    }
    ...
    if (!hadError) {
      conn.emit(
        "close",
        new MongoNetworkError(
          `connection ${conn.id} to ${conn.address} closed`
        ),
        conn
      );
    }
      ...
      if (conn.bytesRead > 0 && conn.sizeOfMessage > 0) {
        ...
        if (conn.stubBuffer != null && conn.stubBuffer.length > 0) {
          ...
          if (conn.stubBuffer.length + data.length > 4) {
            ...
            const newStubBuffer = Buffer.alloc(
              conn.stubBuffer.length + data.length
            );
