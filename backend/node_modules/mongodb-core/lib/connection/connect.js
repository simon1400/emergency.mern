  ...
  return makeConnection(6, options, (err, ipv6Socket) => {
    if (err) {
      makeConnection(4, options, (err, ipv4Socket) => {
        ...
        performInitialHandshake(
          new Connection(ipv4Socket, options),
          options,
          callback
        );
      });
    ...
    performInitialHandshake(
      new Connection(ipv6Socket, options),
      options,
      callback
    );
  });
...
function getSaslSupportedMechs(options) {
  ...
  if (
    typeof authMechanism === "string" &&
    authMechanism.toUpperCase() !== "DEFAULT"
  ) {
    return {};
  }
...
function parseSslOptions(family, options) {
  ...
  for (const name in options) {
    if (
      options[name] != null &&
      LEGAL_SSL_SOCKET_OPTIONS.indexOf(name) !== -1
    ) {
...
function runCommand(conn, ns, command, options, callback) {
  ...
  function errorHandler(err) {
    ...
    CONNECTION_ERROR_EVENTS.forEach(eventName =>
      conn.removeListener(eventName, errorHandler)
    );
  ...
  CONNECTION_ERROR_EVENTS.forEach(eventName =>
    conn.once(eventName, errorHandler)
  );
