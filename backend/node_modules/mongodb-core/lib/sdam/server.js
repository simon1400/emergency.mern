...
class Server extends EventEmitter {
  ...
  connect(options) {
    ...
    if (
      this.s.pool &&
      !this.s.pool.isDisconnected() &&
      !this.s.pool.isDestroyed()
    ) {
      throw new MongoError(
        `Server instance in invalid state ${this.s.pool.state}`
      );
    }
...
function basicReadValidations(server, options) {
  ...
  if (
    ...
    return new MongoError(
      "readPreference must be an instance of ReadPreference"
    );
  }
}
  ...
  return function(pool, conn) {
    ...
    if (server.s.logger.isInfo()) {
      server.s.logger.info(
        `server ${server.name} connected with ismaster [${JSON.stringify(
          ismaster
        )}]`
      );
    }
