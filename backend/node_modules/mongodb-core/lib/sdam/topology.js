...
const readPreferenceServerSelector = require("./server_selectors")
  .readPreferenceServerSelector;
const writableServerSelector = require("./server_selectors")
  .writableServerSelector;
const isRetryableWritesSupported = require("../topologies/shared")
  .isRetryableWritesSupported;
const Cursor = require("../cursor");
const deprecate = require("util").deprecate;
const BSON = require("../connection/utils").retrieveBSON();
const createCompressionInfo = require("../topologies/shared")
...
class Topology extends EventEmitter {
  ...
  connect(options, callback) {
    ...
    this.emit(
      "topologyOpening",
      new monitoring.TopologyOpeningEvent(this.s.id)
    );
    ...
    servers.forEach(server =>
      destroyServer(server, this, () => {
        ...
        if (destroyed === servers.size) {
          ...
          this.emit(
            "topologyClosed",
            new monitoring.TopologyClosedEvent(this.s.id)
          );
  ...
  insert(ns, ops, options, callback) {
    executeWriteOperation(
      { topology: this, op: "insert", ns, ops },
      options,
      callback
    );
  }
  ...
  update(ns, ops, options, callback) {
    executeWriteOperation(
      { topology: this, op: "update", ns, ops },
      options,
      callback
    );
  }
  ...
  remove(ns, ops, options, callback) {
    executeWriteOperation(
      { topology: this, op: "remove", ns, ops },
      options,
      callback
    );
  }
...
function destroyServer(server, topology, callback) {
  ...
  server.destroy(() => {
    topology.emit(
      "serverClosed",
      new monitoring.ServerClosedEvent(
        topology.s.id,
...
function selectServers(topology, selector, timeout, start, callback) {
  ...
  if (descriptions.length) {
    const servers = descriptions.map(description =>
      topology.s.servers.get(description.address)
    );
