...
class AuthProvider {
  ...
  auth(sendAuthCommand, connections, credentials, callback) {
      ...
      this._authenticateSingleConnection(
        ...
        (err, r) => {
          ...
          if (numberOfValidConnections > 0) {
            ...
            if (errorObject == null) {
              errorObject = new MongoError(
                `failed to authenticate using ${credentials.mechanism}`
              );
            }
    ...
    const executeInNextTick = _connection =>
      process.nextTick(() => execute(_connection));
   ...
   */
  logout(source) {
    this.authStore = this.authStore.filter(
