...
class Transaction {
  ...
  constructor(options) {
    ...
    if (options.writeConcern || typeof options.w !== "undefined") {
      ...
      this.options.writeConcern = options.writeConcern
    ...
    if (options.readPreference)
      this.options.readPreference = options.readPreference;
  ...
  get isActive() {
    return (
      [TxnState.STARTING_TRANSACTION, TxnState.TRANSACTION_IN_PROGRESS].indexOf(
        this.state
      ) !== -1
    );
  }
  ...
  transition(nextState) {
    ...
    throw new MongoError(
      `Attempted illegal state transition from [${
        this.state
      }] to [${nextState}]`
    );
  }
