...
function command(server, ns, cmd, options, callback) {
  ...
  if (cmd == null) {
    return callback(
      new MongoError(`command ${JSON.stringify(cmd)} does not return a cursor`)
    );
  }
  ...
  const inTransaction =
    session && (session.inTransaction() || isTransactionCommand(finalCmd));
...
function hasSessionSupport(topology) {
  ...
  return topology.ismaster == null
    ? false
    : topology.ismaster.maxWireVersion >= 6;
}
...
function supportsOpMsg(topologyOrServer) {
  ...
  return (
    description.maxWireVersion >= 6 &&
    description.__nodejs_mock_server__ == null
  );
}
