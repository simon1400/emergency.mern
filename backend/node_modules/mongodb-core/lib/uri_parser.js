...
function checkTLSOptions(queryString) {
  ...
  const tlsValue = assertTlsOptionsAreEqual(
    "tls",
    queryString,
    queryStringKeys
  );
  const sslValue = assertTlsOptionsAreEqual(
    "ssl",
    queryString,
    queryStringKeys
  );
  ...
  if (tlsValue != null && sslValue != null) {
    if (tlsValue !== sslValue) {
      throw new MongoParseError(
        "All values of `tls` and `ssl` must be the same."
      );
    }
  }
}
    ...
    if (Array.isArray(queryString[optionName])) {
      ...
      queryString[optionName].forEach(tlsValue => {
        if (tlsValue !== firstValue) {
          throw new MongoParseError(
            "All values of ${optionName} must be the same."
          );
        }
      });
    }
