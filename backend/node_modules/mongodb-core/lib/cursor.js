...
Cursor.prototype.readBufferedDocuments = function(number) {
  ...
  if (
    ...
    elements = elements.slice(
      0,
      this.cursorState.limit - this.cursorState.currentLimit
    );
    this.kill();
  }

  // Adjust current limit
  this.cursorState.currentLimit =
    this.cursorState.currentLimit + elements.length;
...
var isConnectionDead = function(self, callback) {
  if (self.pool && self.pool.isDestroyed()) {
    ...
    const err = new MongoNetworkError(
      f(
        "connection to host %s:%s was destroyed",
        self.pool.host,
        self.pool.port
      )
    );
...
function initializeCursor(cursor, callback) {
  ...
  if (!cursor.topology.isConnected(cursor.options)) {
    ...
    if (
      cursor.topology._type === "server" &&
      !cursor.topology.s.options.reconnect
    ) {
  ...
  return cursor.topology.selectServer(serverSelectOptions, (err, server) => {
    ...
    if (collationNotSupported(cursor.server, cursor.cmd)) {
      return callback(
        new MongoError(
          `server ${cursor.server.name} does not support collation`
        )
      );
    }
    ...
    const queryCallback = (err, r) => {
      ...
      if (
        ...
        (!cursor.cmd.find ||
          (cursor.cmd.find && cursor.cmd.virtual === false)) &&
        ...
        if (
          result.documents[0].cursor != null &&
          typeof result.documents[0].cursor !== "string"
        ) {
          ...
          if (Array.isArray(result.documents[0].cursor.firstBatch)) {
            cursor.cursorState.documents =
              result.documents[0].cursor.firstBatch; //.reverse();
          }
      ...
      if (
        ...
        cursor.cursorState.documents = cursor.cursorState.transforms.query(
          result
        );
      }
    ...
    if (cursor.logger.isDebug()) {
      cursor.logger.debug(
        `issue initial query [${JSON.stringify(
          cursor.cmd
        )}] with flags [${JSON.stringify(cursor.query)}]`
      );
    }
