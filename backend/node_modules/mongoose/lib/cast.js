    ...
    if (path === "$or" || path === "$nor" || path === "$and") {
      ...
      if (!schematype) {
        ...
        while (j--) {
          ...
          if (
            ...
            const discriminatorVal = get(
              obj,
              pathFirstHalf + "." + discriminatorKey
            );
            if (discriminatorVal != null) {
              schematype = _schematype.schema.discriminators[
                discriminatorVal
              ].path(pathLastHalf);
            }
          }
      ...
      if (!schematype) {
        ...
        if (options.upsert && options.strict) {
          ...
          throw new StrictModeError(
            path,
            'Path "' + path + '" is not in ' + "schema and strictQuery is true."
          );
        ...
        if (!any$conditionals) {
          ...
          while (k--) {
            ...
            if ($cond === "$not") {
              ...
              cast(
                schematype.caster ? schematype.caster.schema : schema,
                nested,
                options,
                context
              );
        ...
        Array.isArray(val) &&
        ["Buffer", "Array"].indexOf(schematype.instance) === -1
      ) {
        const casted = [];
