  ...
  model.hooks.execPre("find", query, () => {
    ...
    model.collection.find(query._conditions, options, function(err, cursor) {
      if (_this._error) {
        ...
        _this.listeners("error").length > 0 &&
          _this.emit("error", _this._error);
      }
      if (err) {
...
function _next(ctx, cb) {
  ...
  if (ctx._transforms.length) {
    callback = function(err, doc) {
      ...
      cb(
        err,
        ctx._transforms.reduce(function(doc, fn) {
...
function _create(ctx, doc, populatedIds, cb) {
  ...
  const opts = populatedIds ? { populated: populatedIds } : undefined;
