...
var find_package_json_with_name = function(name) {
  ...
  while (currentModule) {
    ...
    if (
      !object.peerOptionalDependencies ||
      (object.peerOptionalDependencies &&
        !object.peerOptionalDependencies[parts[0]])
    ) {
      currentModule = currentModule.parent;
      continue;
    }
...
var require_optional = function(name, options) {
  ...
  var dependentOnModule = JSON.parse(
    fs.readFileSync(f("%s/package.json", location))
  );
  // Get the version
  ...
  if (semver.satisfies(version, expectedVersions) == false && options.strict) {
    var error = new Error(
      f(
        "optional dependency [%s] found but version [%s] did not satisfy constraint [%s]",
        parts[0],
        version,
        expectedVersions
      )
