...
_humans to write and maintain_ by hand. It does this by adding some minimal
...
JSON5 is _not_ an official successor to JSON, and JSON5 content may _not_
work with existing JSON parsers. For this reason, JSON5 files use a new .json5
extension. _(TODO: new MIME type needed too.)_
...
JSON isn’t the friendliest to _write_. Keys need to be quoted, objects and
...
has expanded beyond _machines_. JSON is now used for writing [configs][ex1],
[manifests][ex2], even [tests][ex3] — all by _humans_.
...
- Object keys can be unquoted if they’re valid [identifiers][mdn_variables].
  ...
  _(TODO: Unicode characters and escape sequences aren’t yet supported in this
  implementation.)_
...
`JSON5.parse` supports all of the JSON5 features listed above (_TODO: except
Unicode_), as well as the native [`reviver` argument][json-parse].
...
as well. _(TODO: Any implemented `toJSON` methods aren’t used today.)_
